...
DROP POLICY IF EXISTS "own garden scans select" ON public.garden_scan_sessions;
CREATE POLICY "own garden scans select" ON public.garden_scan_sessions
  FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "own garden scans insert" ON public.garden_scan_sessions;
CREATE POLICY "own garden scans insert" ON public.garden_scan_sessions
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "own garden scans update" ON public.garden_scan_sessions;
CREATE POLICY "own garden scans update" ON public.garden_scan_sessions
  FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "own garden scans delete" ON public.garden_scan_sessions;
...
DROP POLICY IF EXISTS "own garden scan events select" ON public.garden_scan_events;
CREATE POLICY "own garden scan events select" ON public.garden_scan_events
  FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "own garden scan events insert" ON public.garden_scan_events;
...
DROP POLICY IF EXISTS "garden scans owner select" ON storage.objects;
CREATE POLICY "garden scans owner select"
  ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'garden-scans' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "garden scans owner insert" ON storage.objects;
CREATE POLICY "garden scans owner insert"
  ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'garden-scans' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "garden scans owner update" ON storage.objects;
CREATE POLICY "garden scans owner update"
  ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'garden-scans' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "garden scans owner delete" ON storage.objects;
